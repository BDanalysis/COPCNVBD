...
#read the bam file，return the list of chrom
def read_bam_file(filename):
    samfile = pysam.AlignmentFile(filename, "rb")
    chrList = samfile.references
    return chrList

#input the file of fasta，return a two-dim list including all information of the bam file
...
def modeRD(RD):
    ...
    count = np.bincount(newRD)
    print(len(count))
    countList = np.full(len(count) - 9, 0)
    for i in range(len(countList)):
        countList[i] = np.mean(count[i:i + 10])
    modemin = np.argmax(countList)
    modemax = modemin + 10
    mode = (modemax + modemin) / 2
    mode = mode / 1000
...
def Read_seg_file(num_col, num_bin, segpath):
    """
